...
namespace RazonesFinancieras.Razones_de_actividad
{
    public partial class RazonDeCuentasPorCobrarForm : Form
    {
        ...
        private void EvaluarButton_Click(object sender, EventArgs e)
        {
            ...
            Ventastxt.Clear();
            CuentasPorCobrartxt.Clear();
            RotacionDeCuentasPorCobrartxt.Clear();
                        ...
                        using (SqlDataReader reader = cmd.ExecuteReader())
                        {
                            if (reader.Read())
                            {
                                ...
                                if (ventas == 0 || cuentasporcobrar == 0)
                                {
                                    ...
                                    if (ventas == 0) mensajeError += "\n- Las ventas son cero o no existen.";
                                    if (cuentasporcobrar == 0) mensajeError += "\n- Las cuentas por cobrar son cero o no existen.";
